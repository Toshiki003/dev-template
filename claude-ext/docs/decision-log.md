...
### 2026-03-01: PRサマリ生成のCodex統合

**決定**: `pr-summary.yml`（外部LLM API依存）を削除し、`codex-review-comment.yml` のコメント本文に「要約 + レビュー」の両方を依頼する形に統合する。これにより `LLM_API_KEY` / `LLM_API_BASE` / `LLM_MODEL` の3つのSecrets/Variablesが不要になる。

**理由**:
- 1PRあたり数千トークン程度の軽微な処理であり、既に契約済みのCodexで十分賄える
- 外部LLMプロバイダーのアカウント管理・APIキー管理が不要になり、運用が大幅に簡素化される
- Secret管理のセキュリティリスクが減少する（外部へのコード送信がなくなる）
- テンプレート利用者の初期設定手順が簡略化される（`AI_ENABLED` のみで動作）

**代替案**:
- **外部LLM APIを維持しCodexレビューと併用**: PRサマリは外部LLM、レビューはCodexの二本立て
  - 却下理由: 機能が重複し、2つのAIサービスの管理コストが不要に高くなるため
- **Codexサマリ + 外部LLMレビューに反転**: サマリをCodex、レビューを外部LLMに移行
  - 却下理由: Codexが両方を処理可能であり、外部依存を残す理由がないため
